...
class CarlaEnv(gym.Env):
    ...
    def reset(self):
        ...
        client.set_timeout(10.0)
        self.world = client.load_world(self.params['town'])
        print('Carla server connected!')
    ...
    def _try_spawn_random_vehicle_at(self, transform, number_of_wheels=[4], autopilot_bool=True):
    ...
    def _get_obs(self):
        ...
        for i in range(self.obs_size):
            for j in range(self.obs_size):
                if abs(birdeye[i, j, 0] - 0) < 20 and abs(birdeye[i, j, 1] - 255) < 20 and abs(
                        birdeye[i, j, 2] - 0) < 20:
